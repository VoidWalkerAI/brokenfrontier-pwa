/* ============================================================
   ...
   PLAYABLE SPINE 001 / RUNTIME PROOF 004:
   Keeps the player interface free-form, prevents duplicate opening
   seeds, and sends visible state to IPC without exposing hidden truth.
   ============================================================ */
...
(function () {
  ...
  const TATTOO = `IPC ENGINE • BUILD 2026-08-30 • PLAYABLE SPINE 001 • RUNTIME PROOF 004 • ${new Date().toISOString()}`;
  ...
  const ui = { tab: "play", pendingRoll: null, inFlight: false, autoToken: 0, openingSeedScheduled: false };
  ...
  try {
    ...
    // ========================================================
    // 🪨 CAVECODE — LOCKED BLOCK
    // ONE-TIME OPENING SEED
    //
    // A fresh save gets one opening request. A reload while that request
    // is in flight must not create a second autonomous vignette.
    // ========================================================
    const transcriptEmpty = !Array.isArray(s.campaign?.transcript) || s.campaign.transcript.length === 0;
    const openingAlreadyPresented = !!s.campaign?.flags?.openingPresented;
    const seedKey = `bf_opening_seed_${String(s.id || "active")}`;
    const lastSeedAt = Number(sessionStorage.getItem(seedKey) || 0);
    const seedRecentlyRequested = Number.isFinite(lastSeedAt) && (Date.now() - lastSeedAt) < 15000;

    if (transcriptEmpty && !openingAlreadyPresented && !seedRecentlyRequested && !ui.openingSeedScheduled) {
      ui.openingSeedScheduled = true;
      sessionStorage.setItem(seedKey, String(Date.now()));
      setTimeout(async () => {
        const fresh = safeGetActiveSave();
        const stillEmpty = !Array.isArray(fresh.campaign?.transcript) || fresh.campaign.transcript.length === 0;
        const stillNeedsOpening = !fresh.campaign?.flags?.openingPresented;
        if (fresh.id === s.id && stillEmpty && stillNeedsOpening) {
          try {
            await gmTurn({ type: "nudge", text: "Begin the campaign." });
          } finally {
