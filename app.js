/* app.js — Broken Frontier RPG (AI-GM Terminal + Saves)
   ...
   FIX v3 (Transcript Render + Patch-Safe):
   - GM responses are guaranteed to land in save.campaign.transcript
   - Transcript survives patch application even if patch nukes campaign
   - Render always re-reads the committed save (no stale object)
*/

(function () {
  const TATTOO = "BUILD 2026-02-19 — TATTOO V1.2 — Transcript render lock";
  ...
  async function gmTurn(event) {
    ...
    // Reload save fresh (avoid stale object problems)
    save = safeGetActiveSave();

    // Harden campaign again (covers reset/import/patch edge cases)
    save.campaign = save.campaign || {};
    save.campaign.transcript = Array.isArray(save.campaign.transcript) ? save.campaign.transcript : [];

    // Keep a safe copy of transcript in case patch wipes it
    ...
    // Apply patch, then re-harden + restore transcript if needed
