...
(function () {
  ...
  const TATTOO = "BUILD 2026-02-19 — TATTOO V1.3 — Display lock (single instance)";
  ...
  // Accept either:
  //  1) window.BF_DB (preferred bundled API)
  //  2) legacy globals (window.loadDB, window.writeDB, ...)
  const dbApi = window.BF_DB || (
    (typeof window.loadDB === "function" && typeof window.writeDB === "function")
      ? {
  ...
  if (!dbApi) {
    $app.innerHTML = `<pre style="padding:16px;color:#fff;background:#0b0b0f;">FATAL — save.js not loaded (no BF_DB + no global helpers)</pre>`;
    return;
  }
